...
* ложным логическим значением являются:
	* значение `nil`;
	* число 0 (целое или вещественное);
	* пустой список;
* истинным логическим значением являются:
	* числа, отличные от 0 (целые или вещественные);
	* непустые списки;
	* любые паки;
	* любые замыкания.
...
В качестве переменных используются замыкания вида `fn name() value;`.

Присваивание осуществляется через переопределение замыканий.
