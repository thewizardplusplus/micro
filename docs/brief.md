...
* символы &mdash; `/'(\\.|[^'])'/`, например: `'t'`, `'\''`;
* строки &mdash; `/"(\\.|[^"])*"/`, например: `""`, `"test"`, `"Say \"Hello!\"."`.
...
Если имя переменной указано, то данная переменная доступна посредством этого имени ниже в текущей области видимости, а также внутри определения самой себя.
