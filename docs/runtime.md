...
* системные функции:
	* `load(filename: str): any` &mdash; загружает файл скрипта `filename`, выполняет его и возвращает полученный результат; данный результат кешируется по абсолютному пути к загруженному файлу;
